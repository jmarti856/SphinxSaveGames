                ...
                try
                {
                    foreach (string s in ArchivoFinal)
                    {
                        if (!s.StartsWith("/*"))
                        {
                            ...
                            {
                                tw.WriteLine("Setobjective " + cadenaDividida[0] + " " + cadenaDividida[1]);
                            }
                            if (cadenaDividida[0].StartsWith("HT_Item"))
                            {
                                if (!cadenaDividida[1].Equals("-1"))
                                {
                                    tw.WriteLine("InventorySet " + cadenaDividida[0] + " " + cadenaDividida[1]);

                                    if (cadenaDividida[0].Equals("HT_Item_Pickup_BronzeScarab") || cadenaDividida[0].Equals("HT_Item_Weapon_IceDart") ||
                                    cadenaDividida[0].Equals("HT_Item_Pickup_MummyMoney") || cadenaDividida[0].Equals("HT_Item_Weapon_AcidDart") ||
                                    cadenaDividida[0].Equals("HT_Item_Weapon_FireDart"))
                                    {
                                        tw.WriteLine("InventoryMaxSet " + cadenaDividida[0] + " " + cadenaDividida[2]);
                                    }
                                }
                            }
                        }
